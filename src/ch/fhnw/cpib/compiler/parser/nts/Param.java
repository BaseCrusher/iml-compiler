...
public class Param implements INtsParser {
    ...
    public Param(Environment environment) throws GrammarError {
        ...
        if (token.hasTerminal(IDENT, CHANGEMODE, MECHMODE, FLOWMODE)) {
            ...
            Variable variable = new Variable(typedIdent.getIdentifier().getValue(), optFlowmode, optMechmode, optChangemode, typedIdent.getType(), environment.getStartAddress() + environment.getVars().size());
